...
- `COURSE_DATA`：`COURSE_DIR/.course/`，所有课程数据的存放根目录

所有课程数据（config、state、memory、uploads、processed）均在 `COURSE_DATA`（即 `COURSE_DIR/.course/`）下。
所有 skill 代码（ui/、commands/、prompts/）均在 `SKILL_DIR` 下。

课程目录结构示例：
```
physics-101/          ← COURSE_DIR（用户工作目录，保持干净）
└── .course/          ← COURSE_DATA（所有托管数据）
    ├── config.json
    ├── state/
    ├── calendar/
    ├── memory/
...
bootstrap.py 会：
1. 检测 `COURSE_DATA` 是否是合法课程文件夹（存在 `.course/config.json`）
2. 读取 `.course/state/session.json`（当日缓存）
3. 读取 `.course/memory/index.json`（长期记忆索引）
...
处理完成后，将提取内容按下方 Evoldown 格式写入 `.course/processed/knowledge_base/{topic}.json`。
...
每个知识点存为独立 JSON 文件 `.course/processed/knowledge_base/{topic}.json`：
...
写入 `.course/memory/mistakes/{date}_{topic}.json` 的触发条件：
- 用户答题出错
...
同步更新 `.course/memory/index.json`，追加一条索引记录，供 bootstrap 快速检索。
